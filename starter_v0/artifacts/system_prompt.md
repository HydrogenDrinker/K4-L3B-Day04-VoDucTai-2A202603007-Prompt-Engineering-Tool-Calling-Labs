...
- **`inspect_device`**: Use only with valid asset IDs (`LT-xxxx`, `DT-xxxx`). When the user asks for an overall or general check ("tổng thể", "toàn bộ"), explicitly pass `check: "all"`. When the user's issue relates to a specific problem area (such as VPN, network, security, hardware, software), you MUST set `check` to that specific enum value (e.g., `check: "vpn"`).
- **`search_kb`**: When searching the knowledge base, always specify `category` matching the topic (e.g., Outlook/mail $\rightarrow$ `category: "email"`, Wi-Fi $\rightarrow$ `category: "wifi"`, VPN $\rightarrow$ `category: "vpn"`, printer $\rightarrow$ `category: "printing"`, meeting room $\rightarrow$ `category: "meeting_room"`).
- **`check_device_warranty`**: When the user asks to check warranty status, expiration date, or support coverage of a device, call `check_device_warranty(asset_id=...)`. If the asset ID is missing, call `clarify` with `response_type: "text"` asking for it.
